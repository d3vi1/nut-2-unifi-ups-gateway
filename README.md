...
same GitHub Release trust root. Every release requires a public repository and
GHCR package, GitHub immutable releases, a protected `main`, and a no-bypass
`v*` ruleset that blocks tag updates and deletion. The release controller runs
only from the live `main` workflow, atomically reserves the version tag and
creates its owned draft before the image, and rejects automatic reruns or any
pre-existing state for that version.
...
| An observed-symptom case is an already-adopted UPS renamed in Network that remains **Getting Ready** | First confirm that `/readyz` succeeds and the inform connection has no errors. Some plain-HTTP Network versions repeatedly send an authenticated GCM configuration marker after a rename. On a trusted management LAN only, the default-off `N2U_UNIFI_HTTP_GCM_VOLATILE_CFGVERSION_SYNC=true` option is intended to mirror that marker in memory. Whether it clears **Getting Ready** remains **CANDIDATE** until exact-build live acceptance; see [Configuration](docs/configuration.md#plain-http-gcm-cfgversion-compatibility). |
...
| The volatile option leaves the UPS in Getting Ready | It rejects ordinary multi-field management responses. The separate `N2U_UNIFI_HTTP_GCM_CONFIG_RECEIPT_MODE=memory` experiment accepts the observed shape; after live acceptance, `persistent` retains its marker across restart. Keep the volatile option false. Both remain **CANDIDATE**, and neither applies controller settings. Follow [Configuration receipts](docs/configuration.md#multi-field-configuration-receipts). |
| `/readyz` reports a receipt storage error | Repair the private receipt file or volume, then restart. Adoption remains in `state.json`; avoid resetting or deleting the state volume. |
| Network repeatedly schedules a firmware upgrade | The gateway cannot install Ubiquiti firmware. The optional [reported firmware mirror](docs/configuration.md#controller-selected-reported-firmware) remembers Network's selected version, including a lower version after a channel change, without downloading anything. It requires persistent configuration receipts and a trusted management LAN; exact-build convergence remains **CANDIDATE**. |
...
The mutable `edge` tag is available only for development from `main`.
Releases intentionally publish no human-readable SemVer or `latest` alias.
They retain a unique internal run-scoped tag so the manifest remains reachable,
but the supported installation executes only the digest-pinned reference from
...
The release controller publishes a multi-platform image with an SBOM and build
