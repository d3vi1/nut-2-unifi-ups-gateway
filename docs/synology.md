...
| Getting Ready persists with the volatile option | Multi-field management responses need the separate `N2U_UNIFI_HTTP_GCM_CONFIG_RECEIPT_MODE=memory` experiment, with the volatile option false. Use `persistent` only after confirming convergence. Receipt modes remain **CANDIDATE** until live acceptance; see [Configuration receipts](configuration.md#multi-field-configuration-receipts). |
| Receipt status is `storage_error` | Repair the private receipt file or volume and restart. Preserve the adoption file and named volume; no adoption reset is required. |
